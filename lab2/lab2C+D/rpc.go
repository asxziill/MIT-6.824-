...
type ApplyMsg struct {
	CommandValid bool        //命令是否成功
	Command      interface{} //命令
	CommandTerm  int         //命令所在的任期号
	CommandIndex int         //命令在日志中的索引

	SnapshotValid bool   //是否包含一个有效的快照
	Snapshot      []byte //快照数据
	SnapshotTerm  int    //快照所对应的任期
	SnapshotIndex int    //快照所对应的最后一个日志条目的索引
}
...
type InstallSnapshotRequest struct {
	Term              int    //任期
	LeaderId          int    //领导者id
	LastIncludedIndex int    //上一个日志索引 (用来匹配)
	LastIncludedTerm  int    //上一个日志任期
	Data              []byte // 数据
}
